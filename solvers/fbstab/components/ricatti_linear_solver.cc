...
bool RiccatiLinearSolver::Initialize(const MpcVariable& x,
  ...
  if (xbar.data_ != data) {
    throw std::runtime_error(
        "In RiccatiLinearSolver::Factor: x and xbar have mismatched problem "
        "data.");
  }
  if (!MpcVariable::SameSize(x, xbar)) {
    throw std::runtime_error(
        "In RiccatiLinearSolver::Factor: x and xbar are not the same size.");
  }
  if (sigma <= 0) {
    throw std::runtime_error(
        "In RiccatiLinearSolver::Factor: sigma must be positive.");
  }
